...
C             set SW(7),SW(8), and SW(14) equal to 0.
      ...
      DIMENSION AP(*),W(2),WINDF(2),WW(2),SV(25)
...
C-----------------------------------------------------------------------
      ...
      REAL LAT,LONG
      DIMENSION WB(2,15),WC(2,15),PB(*),PC(*),WW(2)
      DIMENSION AP(*)
      COMMON/CSW/SW(25),ISW,SWC(25)
      COMMON/HWMC/WBT(2),WCT(2)
...
C       SUM WINDS AND CHANGE MERIDIONAL SIGN TO + NORTH
   ...
   50 CONTINUE
      ...
      END
C-----------------------------------------------------------------------
c      SUBROUTINE TSELEC(SV)
cC        SET SWITCHES
cC        SW FOR MAIN TERMS, SWC FOR CROSS TERMS
c      DIMENSION SV(*),SAV(25),SVV(1)
c      COMMON/CSW/SW(25),ISW,SWC(25)
c      DO 100 I = 1,25
c        SAV(I)=SV(I)
c        SW(I)=AMOD(SV(I),2.)
c        IF(ABS(SV(I)).EQ.1.OR.ABS(SV(I)).EQ.2.) THEN
c          SWC(I)=1.
c        ELSE
c          SWC(I)=0.
c        ENDIF
...
c      DATA ISDATE/'28-J','AN-9','3   '/,ISTIME/'20:3','5:39'/
c      DATA NAME/'HWM9','3   '/
C         WINF
      DATA PBA1/
